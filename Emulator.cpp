...
auto Emulator::assembleInstruction(std::vector<uint8_t>& program, ResolvedToken* token) -> void {
    std::uint32_t bin{0};
    ...
    uint8_t shamt = (token->args.size() > 3) ? static_cast<uint8_t>(token->args[3]) : 0;
...
auto Emulator::assemble(const std::vector<ResolvedToken*>& tokens) -> std::vector<std::uint8_t> {
...
auto Emulator::printBinaryProgram(const std::vector<std::uint8_t>& program) -> void {
